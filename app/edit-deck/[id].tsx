...
export default function EditDeckScreen() {
  ...
  const toggleCommander = (cardId: string) => {
    ...
    console.log('EditDeckScreen: Toggle commander clicked for:', clickedCard.name);
    console.log('EditDeckScreen: Current commanders:', commanders.length);
    console.log('EditDeckScreen: Current partner commanders:', partnerCommanders.length);
    ...
    setCards(prev => {
      const updated = prev.map(card => {
        if (card.id === cardId) {
          ...
          if (card.isCommander) {
            console.log('EditDeckScreen: Clicking orange flag - turning red');
          ...
          if (card.isPartnerCommander) {
            console.log('EditDeckScreen: Clicking red flag - removing commander status');
          ...
          if (commanders.length === 0 && partnerCommanders.length === 0) {
            console.log('EditDeckScreen: No commanders - making this commander (orange)');
          ...
          if (commanders.length === 0 && partnerCommanders.length === 1) {
            console.log('EditDeckScreen: One partner exists - making this partner (red)');
            return { ...card, isCommander: false, isPartnerCommander: true };
          }
          
          // If there's only a commander (orange), make this partner (red)
          if (commanders.length === 1 && partnerCommanders.length === 0) {
            console.log('EditDeckScreen: One commander exists - making this partner (red)');
            return { ...card, isCommander: false, isPartnerCommander: true };
          }
        }
        return card;
  ...
  const handleUploadFile = async () => {
    try {
      ...
      console.log('EditDeckScreen: Error uploading file:', error);
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.section, { paddingTop: 20 }]}>
        <View style={styles.row}>
          ...
          <Text style={styles.title}>{t.editDeck}</Text>
          <TouchableOpacity
            onPress={handleSave}
